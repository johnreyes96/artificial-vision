...
plt.savefig("../../output/HCColor2_gray.png")
plt.show()
...
histogramaEG = list(
    histogramaEG.flatten())  # https://stackoverflow.com/questions/1966207/convert-numpy-array-to-python-list
...
#
# En ejemplos en la red, valsPixel (vector eje x) puede ser ndarray (longitud,), pero histogramaEG debería ser una
# lista de n ítems. Referencia: https://pythonspot.com/matplotlib-bar-chart/

plt.bar(valsPixel, histogramaEG, align='center', width=1)
plt.xlabel('Valor - píxel')
plt.ylabel('Frecuencia')
plt.title('Histograma de imagen en escala de grises.')
plt.show()

"""(d.) Ejercicio: visualizar los histogramas de cada canal de color de una imagen RGB. Utilice un subplot para poder 
comparar los histogramas por canales de color. Considere la información propuesta en: 
https://matplotlib.org/3.1.1/gallery/lines_bars_and_markers/categorical_variables.html#sphx-glr-gallery-lines-bars
-and-markers-categorical-variables-py"""

fig, axs = plt.subplots(1, 3, figsize=(12, 4), sharey=True)
axs[0].bar(valsPixel, histogramaRC, align='center',
           width=1)  # En el nombre histogramaRC, RC indica que uno se está refiriendo al canal de color ROJO.
axs[1].bar(valsPixel, histogramaGC, align='center', width=1)
axs[2].bar(valsPixel, histogramaBC, align='center', width=1)
plt.show()

"""(e.) El histograma permite argumentar sobre el contraste de una imagen. No obstante, sería conveniente calcular 
alguna métrica de contraste. Considere la siguiente información, y evalúe numéricamente el contraste de imágenes 
etiquetadas como de alto o bajo contraste:
...
def fn_contrast(input_im, contrast_type):
    ...
    if "Luminance" == contrast_type:
        contrast_f = (l_max - l_min) / (l_min + 1)
    elif "Simple" == contrast_type:
        contrast_f = l_max / (l_min + 1)
    elif "Michelson" == contrast_type:
        contrast_f = (l_max - l_min) / (l_max + l_min)
    else:
...
contrastFeat = fn_contrast(imagenEntrada, "Michelson")
print(contrastFeat)
