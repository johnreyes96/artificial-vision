...
plt.rcParams.update({'font.size': 14})  # 19/09/22. Actualización del tamaño de la fuente en títulos y ejes de figuras.
...
def fnHistograma(input_im):
    ...
    vHistograma = list(vHistograma.flatten())  # El histograma se convierte a un archivo de tipo lista.
...
imagenEnt = cv2.imread('../../data/HCColor1.jpg', cv2.IMREAD_GRAYSCALE)
nf, nc = imagenEnt.shape
imagenEnt = imagenEnt.astype(np.float64)  # Debido a que se utilizará covolución, que puede producir matrices con
# punto decimal, se cambia la precisión admisible en los píxeles de las imágenes.

"""Evaluar el efecto que tienen diferentes tipos de ruido, en la información visual de una imagen. Información de 
referencia: https://www.kaggle.com/code/chanduanilkumar/adding-and-removing-image-noise-in-python/notebook

En un primer caso se considera la adición de ruido Gaussiano, denominado así porque su función de distribución de 
probabilidad se asemeja a una función Gaussiana. Para visualizar esta distribución, utilizar la función fnHistograma."""
...
cv2.randn(gauss_noise, 50, 10)  # Acá se especifican el valor promedio de los píxeles en la imagen de ruido,
...
gauss_noiseHist = gauss_noise  # Copia de la imagen con patrón de ruido. Justo debajo, se convierte a tipo uint8 para
...
plt.axis("off")
plt.title("Uniforme")
plt.savefig("../../output/HCColor2_uniform_noise_pattern.png")
plt.show()
...
plt.savefig("../../output/HCColor2_salt_and_pepper_noise.png")
plt.show()
cv2.waitKey(0)
...
fig.add_subplot(2, 2, 3)  # La tercera visualización del subplot es un diagrama de barras con el histograma del
# patrón de ruido.
plt.bar(vecPixels, histRuido, align='center', width=1)
plt.xlabel("Valor - píxel", fontsize=10)  # Referencia de esta línea y la siguiente:
...
plt.savefig("../../output/HCColor2_original_plus_gaussiano_noise.png")
plt.show()
...
plt.savefig("../../output/HCColor2_original_average_gaussiano.png")
plt.show()
...
plt.savefig("../../output/HCColor2_median_filter.png")
plt.show()
